...
to_si = PatternMatcher([
  ...
  # once we load a value from image buffer, it becomes the base dtype
  (UPat(tuple(Ops), name="x"), lambda x: x.replace(dtype=x.dtype.base) if isinstance(x.dtype, ImageDType) and x.op is not Ops.BUFFER else None),
])
...
def load_realized(ctx:ScheduleContext, b:UOp, st:UOp):
  ...
  return UOp(Ops.PRELOAD if b in ctx.assigns else Ops.LOAD, st.dtype, (b, unwrap(st.st).to_uop()))
...
break_sched = PatternMatcher([
  ...
  (UPat(Ops.CONST, name="x", src=(UPat(Ops.VIEW, name="st"),)), lambda x,st: x.replace(src=()).valid(st.st)),
