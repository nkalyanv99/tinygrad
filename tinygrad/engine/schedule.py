...
def to_uop(buf:UOp, ctx:ScheduleContext, cache:Dict[UOp, UOp]) -> UOp:
  ...
  if buf.st is None or buf.base.is_realized or (buf.op is Ops.VIEW and buf.base.src[1].op in {Ops.CONST, Ops.BIND}): return buf
  # view is passthrough
...
def generate_valid(ctx:ScheduleContext, st:UOp, const:UOp) -> UOp:
...
def create_schedule_with_vars(outs:List[UOp]) -> Tuple[List[ScheduleItem], Dict[Variable, int]]:
  if len(outs:=dedup(x.base for x in outs if x.base.realized is None and not (x.base.op is Ops.VIEW and x.base.src[1].op in {Ops.CONST, Ops.BIND}))) == 0: return [], {}
